...
class ProductController extends AdminController{
    ...
    public function EditPost(Request $request, Product $product) {
        ...
        if($request->file('video_image')){
            $image = $product->video_image;
            $publicId = substr($image, 0 ,strrpos($image, ".")); 
            if(!empty($image)) {
                Cloudder::delete($publicId);
            }
            $image_name = $request->file('video_image')->getRealPath();
            Cloudder::upload($image_name, null);
            $imagereturned = Cloudder::getResult();
            $image_id = $imagereturned['public_id'];
            $image_format = $imagereturned['format'];    
            $image_new_name = $image_id.'.'.$image_format;
            $product_post['video_image'] = $image_new_name;
        }
    ...
    public function fetch_category_products(Category $category) {
        $rows = Product::where('category_id', $category->id)->with('images', 'category', 'store', 'multiOptionss')->get();
        $data = json_decode(($rows));
        // dd($data[0]);
        return response($data, 200);
    }
