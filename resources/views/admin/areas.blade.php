                ...
                <table id="without-print" class="table table-hover non-hover" style="width:100%">
                    <thead>
                        <tr>
                            ...
                            <th class="text-center">{{ __('messages.delivery_costs') }}</th>
                        ...
                        @foreach ($data['areas'] as $area)
                            <tr>
                                ...
                                <td class="text-center">
                                    <a href="{{ route('areas.add.delivercost', $area->id) }}" target="_blank">{{ __('messages.add') }}</a> | <a href="{{ route('areas.show.delivercost', $area->id) }}" target="_blank">{{ __('messages.show') }}</a>
                                </td>
                                @if(Auth::user()->update_data) 
