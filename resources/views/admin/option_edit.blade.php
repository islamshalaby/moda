    ...
    <div class="col-lg-12 col-12 layout-spacing">
        ...
        <form action="" method="post" enctype="multipart/form-data" >
            ...
            <div class="form-group">
                <label for="category_select">{{ __('messages.property_category') }}</label>
                <select id="category_select" name="property_category_id" class="form-control">
                    <option selected disabled >{{ __('messages.select') }}</option>
                    @foreach ( $data['properties_categories'] as $cat )
                    <option {{ $cat->id == $data['option']['property_category_id'] ? 'selected' : '' }} value="{{ $cat->id }}">{{ App::isLocale('en') ? $cat->title_en : $cat->title_ar }}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group">
