                    ...
                    <tbody>
                        ...
                        @foreach ($data['categories'] as $category)
                            <tr>
                                ...
                                @if(Auth::user()->update_data) 
                                    <td class="text-center blue-color" ><a href="{{ route('options.categories.edit', $category->id) }}" ><i class="far fa-edit"></i></a></td>
                                @endif
                                @if(Auth::user()->delete_data) 
                                    <td class="text-center blue-color" >
                                        @if(count($category->options) == 0)
                                        <a onclick='return confirm("{{ __('messages.are_you_sure') }}");' href="{{ route('options.categories.delete', $category->id) }}" ><i class="far fa-trash-alt"></i></a>
                                        @else
