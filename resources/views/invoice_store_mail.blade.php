    ...
    <div class="invoice-box" style="max-width: 800px;margin: auto;padding: 30px;border: 1px solid #eee;box-shadow: 0 0 10px rgba(0, 0, 0, .15);font-size: 16px;line-height: 24px;font-family: 'Helvetica Neue', 'Helvetica', Helvetica, Arial, sans-serif;color: #555;">
        <table cellpadding="0" cellspacing="0" style="width: 100%;line-height: inherit;text-align: left;">
                    ...
                    <table style="width: 100%;line-height: inherit;text-align: right;">
                        <tr>
                            <td class="title" style="padding: 5px;vertical-align: top;padding-bottom: 20px;font-size: 45px;line-height: 45px;color: #333;">
                                <img src="https://res.cloudinary.com/dk1fceelj/image/upload/h_200,w_200/v1581928924/{{ $order->store['logo'] }}" style="width:100px; max-width:300px;">
                            </td>
                            
                            <td style="padding: 5px;vertical-align: top;text-align: left;padding-bottom: 20px;">
                                {{ __('messages.invoice') }} : {{ $order->main['main_order_number'] }}<br>
                                {{ __('messages.date') }}: {{ $order['created_at']->format("d-m-y") }}<br>
                            </td>
                        </tr>
                    </table>
                </td>
                    ...
                    <table style="width: 100%;line-height: inherit;text-align: right;">
                        <tr>
                            <td style="padding: 5px;vertical-align: top;padding-bottom: 40px;">
                                ...
                                {{ $order->main->user->name }}<br>
                                <a style="text-decoration: none" href="https://www.google.com/maps/?q={{ $order->main->address ? $order->main->address->latitude : '' }},{{ $order->main->address ? $order->main->address->longitude : '' }}" target="_blank"> {{ $order->main->address->area ? $order->main->address->area->title_en . ", " . __('messages.st') . " " . $order->main->address->street . ", " . __('messages.piece') . " " . $order->main->address->piece . ", " . __('messages.gaddah') . " " . $order->main->address->gaddah : '' }} <br/> {{ __('messages.home') . " " . $order->main->address->building . ', ' . __('messages.floor') . " "  . $order->main->address->floor . ', ' . __('messages.apartment') . " " . $order->main->address->apartment_number }}</a><br>
                                <br><br>
                                <h6>{{ __('messages.additional_details') }}</h6>
                                {{ $order->main->address->extra_details }}
                            </td>
                            
                            <td style="padding: 5px;vertical-align: top;text-align: left;padding-bottom: 40px;">
                                <h4 style="margin-bottom: 50px;"></h4><br>
                                {{ $order->main->user->phone }}<br>
                                {{ $order->main->user->email }}
                            </td>
                        </tr>
            ...
            <tr class="heading">
                ...
                <td style="padding: 5px;vertical-align: top;text-align:center;background: #eee;border-bottom: 1px solid #ddd;font-weight: bold;">
                    {{ App::isLocale('en') ? $m_option['title_en'] : $m_option['title_ar'] }}
                </td>
                <td style="padding: 5px;vertical-align: top;text-align:center;background: #eee;border-bottom: 1px solid #ddd;font-weight: bold;">
            ...
            <tr class="item">
                ...
                <td style="padding: 5px;vertical-align: top;text-align: center;border-bottom: 1px solid #eee;">
                    @if($item->option_id != 0)
                    {{ App::isLocale("en") ? $item->val_en : $item->val_ar }}
                    @else
                    {{ __('messages.tailoring_clothes') }}
                    @endif
                </td>
                <td style="padding: 5px;vertical-align: top;text-align: center;border-bottom: 1px solid #eee;">
            ...
            @foreach ($order->oItems as $item)
            @if($item->size && !empty($item->size->tall))
            <tr class="information">
                <td colspan="7" style="padding: 5px;vertical-align: top;">
                    <table style="width: 100%;line-height: inherit;text-align: right;">
                        <tr>
                            <td style="padding: 5px;padding-top:0;padding-bottom: 20px;">
                                <h6>{{ App::isLocale('en') ? $item->product->title_en : $item->product->title_ar }}</h6>
                            </td>
            ...
            <tr class="item">
                ...
                <td style="padding: 5px;vertical-align: top;text-align: left;border-bottom: 1px solid #eee;">
                    ...
                    @if($order->main->payment_method == 1)
                    {{ __('messages.key_net') }}
                    @else
                    {{ __('messages.cash') }}
                    @endif
                ...
                <td style="padding: 5px;vertical-align: top;text-align: right;border-bottom: 0px solid #eee;">
                    <p class="">{{ $order['subtotal_price'] }} {{ __('messages.dinar') }}</p>
                </td>
            </tr>
            <tr class="item">
                <td style="padding: 5px;vertical-align: top;text-align:right;border-bottom: 0px solid #eee;">
                ...
                <td style="padding: 5px;vertical-align: top;text-align: right;border-bottom: 0px solid #eee;">
                    <p class="">{{ $order['delivery_cost'] }} {{ __('messages.dinar') }}</p>
                </td>
            </tr>
